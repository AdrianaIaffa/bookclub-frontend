...
export default function BookClub() {
  ...
  useEffect(() => {
    const getBookClub = async () => {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/bookclubs/`, {
        method: "GET",
        headers: {
        ...
        {bookclubs.map((bookclub, index) => (
          <div key={index} className="bookclub-item">
            ...
            <div className="info-container">
              <h3>
                <Link
                  to={`/bookclubs/${bookclub.url.split("/").reverse()[1]}`}
              ...
              <p className="description">
                ...
                <span
                  className="read-more"
                  onClick={() =>
                    toggleDescription(bookclub.url.split("/").slice(-2, -1)[0])
                  }
                >
                  {expandedDescriptions[
                    bookclub.url.split("/").slice(-2, -1)[0]
                  ]
                    ? "See less"
                    : "Read more"}
                </span>
